...
class Analyzer(
  ...
  object ResolveRelations extends Rule[LogicalPlan] {
    ...
    def apply(plan: LogicalPlan): LogicalPlan = plan resolveOperators {
      case i @ InsertIntoTable(u: UnresolvedRelation, parts, child, _, _, _, _) if child.resolved =>
        ...
        table match {
          case relation: PartitionedRelation if relation.partitionColumns.nonEmpty =>
            ...
            if (parts.keys.nonEmpty) {
              ...
              i.copy(table = EliminateSubQueries(table))
            } else {
              ...
              i.copy(
                table = EliminateSubQueries(table),
  ...
  class ResolveOutputColumns extends Rule[LogicalPlan] {
    ...
    private def expectedColumns(data: LogicalPlan,
      ...
      data match {
        case partitioned: PartitionedRelation =>
          val tablePartitionNames = partitioned.partitionColumns.map(_.name)
