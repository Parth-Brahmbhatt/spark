...
private[history] class FsHistoryProvider(conf: SparkConf, clock: Clock)
  ...
  // TODO: 2 things that I need to think about
  // one we are now not going to insert in the same order so an entry that shouldn't be evicted
  // can be evicted but we are not even using remove eldest entry in any way. may be a sorted
  // map would do? second the justification for the synchronization block and can I update the
  // reference on which I am synchronized?
